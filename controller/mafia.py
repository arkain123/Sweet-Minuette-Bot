...
class Mafia(commands.Cog):
    ...
    async def join(self, inter):
        if self.LEVEL == "PRESTART":
            ...
            self.regplayers[len(self.regplayers)-1].role = "NONE"
            ...
            for i in range(len(self.aliveplayers)):
                if self.aliveplayers[i].name == ctx.guild.categories[1].channels[4+i].name:
                    if self.aliveplayers[i].role == "MARE":
                        ctx.guild.categories[1].channels[4+i].send(f"{ctx.guild.get_member(self.aliveplayers[i].id).mention}, твоя роль - **Мирная кобылка**")
                    if self.aliveplayers[i].role == "DOG":
                        ctx.guild.categories[1].channels[4+i].send(f"{ctx.guild.get_member(self.aliveplayers[i].id).mention}, твоя роль - **Алмазный пёс**")
                        self.mafiaplayers.append(self.aliveplayers[i])
                    if self.aliveplayers[i].role == "NURSE":
                        ctx.guild.categories[1].channels[4+i].send(f"{ctx.guild.get_member(self.aliveplayers[i].id).mention}, твоя роль - **Медсестра**")
                    if self.aliveplayers[i].role == "SHERIFF":
                        ctx.guild.categories[1].channels[4+i].send(f"{ctx.guild.get_member(self.aliveplayers[i].id).mention}, твоя роль - **Шериф**")
                    if self.aliveplayers[i].role == "MANIAC":
                        ctx.guild.categories[1].channels[4+i].send(f"{ctx.guild.get_member(self.aliveplayers[i].id).mention}, твоя роль - **Маньяк**")
                    if self.aliveplayers[i].role == "DEPUTY":
                        ctx.guild.categories[1].channels[4+i].send(f"{ctx.guild.get_member(self.aliveplayers[i].id).mention}, твоя роль - **Помощник шерифа**")
